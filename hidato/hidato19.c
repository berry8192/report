...
int path_dec(struct number *num, int **board, struct position *cboard, int size){
  ...
  for(i=1;i<=size;i++){
	...
	if(flag==1){
	  ...
	  k-=PLIST;
	  num[i].plist[k-1].can=0;
	  num[i].path_number--;
	  printf("dec num[%d].plist[%d].pos[%d]=%d\n",i,j,k-1,num[i].path_number);
	  dec=1;
...
int main(int argc, char **argv){
  ...
  while(1){
    printf("\n");
    print_board(board, sizex, sizey);//check
    if(path_dec(&*num, &*board, &cboard, sizex*sizey)==0 && blank_ume(&*num, &*board, &*cboard, sizex, sizey)==0){
      //printf("cannot slove\n");
      break;
    }
  }
